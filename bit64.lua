-- Lua bit ops for FFI 64-bit numbers. Copyright 2019 Manuel Blanc.

local bit = require("bit")
local band, bor, bxor, bnot = bit.band, bit.bor, bit.bxor, bit.bnot
local lshift, rshift, arshift = bit.lshift, bit.rshift, bit.arshift
local bswap, tohex = bit.bswap, bit.tohex
local TWO32 = require("ffi").new("uint64_t", 2^32)
...
local function U64_join(hi, lo)
    ...
    return (hi * TWO32) + (lo % TWO32)
end

local function U64_split(x)
    return tonumber(x / TWO32),  tonumber(x % TWO32)
end
